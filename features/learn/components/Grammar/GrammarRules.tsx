...
export const GrammarRules: React.FC<GrammarRulesProps> = ({ rules }) => {
  ...
  const hasRules = safeRules.length > 0;
  ...
  if (!hasRules) {
    return (
      <div className="space-y-6">
        ...
        <div className="p-8 border-2 border-slate-200 dark:border-slate-700 border-dashed rounded-2xl bg-slate-50 dark:bg-slate-800/50">
          <div className="text-center text-slate-500 dark:text-slate-400">
            <svg className="w-12 h-12 mx-auto mb-4 opacity-50" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
            </svg>
            <p className="font-medium">Grammar rules coming soon...</p>
          </div>
        </div>
      </div>
    );
